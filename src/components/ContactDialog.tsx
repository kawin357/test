...
import { HelpCircle, Send, MessageCircle, Mail, ExternalLink, X, Phone, MapPin, Clock } from 'lucide-react';
...
const ContactDialog = ({ children, onOpenChange }: ContactDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = encodeURIComponent(
        `Name: ${formData.name}\nEmail: ${formData.email}\n\nMessage:\n${formData.message}\n\n---\nSent via chatz.IO Contact Form`
      );
  ...
  return (
    <Dialog onOpenChange={onOpenChange}>
      ...
      <DialogContent className="w-[95%] sm:w-[600px] max-w-2xl max-h-[90vh] overflow-y-auto border-2 border-white/40 z-[200] rounded-3xl p-0 shadow-2xl"
      style={{
      ...
      >
        <DialogHeader className="relative bg-transparent px-6 sm:px-8 pt-6 pb-4 border-b border-slate-200/30 rounded-t-3xl">
          <DialogTitle className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-emerald-600 via-blue-600 to-cyan-600 bg-clip-text text-transparent">
            Student Support
          </DialogTitle>
          <DialogDescription className="text-sm sm:text-base text-slate-700 mt-2">
